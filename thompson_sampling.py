...
class Thompson:
    ...
    def sampling(self) -> int:
        ...
        if self.random:
            thetas = [np.random.beta(1 + alpha_init + alpha,
                  ...
                  in zip(self.alpha_init, self.rewards2.sum(axis=1),
                         self.beta_init, self.penalties2.sum(axis=1))]
        else:
            thetas = [np.random.beta(1 + alpha_init + alpha,
                                    1 + beta_init + beta)
                    for (alpha_init, alpha, beta_init, beta)
    ...
    def run_experiment(self) -> None:
        ...
        self.rewards2 = np.zeros((self.n_bandits, self.steps))
        self.penalties2 = np.zeros((self.n_bandits, self.steps))
        # variables storing the global progression of the experiment
        ...
        for t in range(self.steps):
            ...
            flag = np.random.binomial(1, 0.9)
            if reward == 1 and flag:
                self.rewards2[bandit, t] += 1 * self.alpha_damping
            elif reward == 1:
                self.penalties2[bandit, t] += 1 * self.beta_damping
            elif reward == 0 and flag:
                self.penalties2[bandit, t] += 1 * self.beta_damping
            elif reward == 0:
                self.rewards2[bandit, t] += 1 * self.alpha_damping
        ...
        self.cumsum_rewards2 = self.rewards2.cumsum(axis=1)
        self.cumsum_penalties2 = self.penalties2.cumsum(axis=1)
        self.total_rewards2 = self.cumsum_rewards2.sum(axis=0)